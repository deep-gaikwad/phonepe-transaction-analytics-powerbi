...
|File|Description|
|-|-|
|📊 PhonePe\_Dataset.xlsx|Raw transaction dataset containing users and payment records|
|📈 PhonePe\_Analytics\_Dashboard.pbix|Complete Power BI dashboard file|
...
```DAX
Total Users =
DISTINCTCOUNT(Dim\_User\[User\_ID])
```
...
```DAX
Total Transactions =
COUNT(Fact\_Transactions\[Transaction\_ID])
```
...
DIVIDE(
    CALCULATE(
        COUNT(Fact\_Transactions\[Transaction\_ID]),
        Fact\_Transactions\[Status]="Success"
    ),
    \[Total Transactions]
)
```
...
```DAX
Average Transaction =
AVERAGE(Fact\_Transactions\[Amount])
```
...
CALCULATE(
    COUNT(Fact\_Transactions\[Transaction\_ID]),
    Fact\_Transactions\[Status]="Failed"
)
```
...
```
PhonePe\_Analytics\_Dashboard.pbix
```
